...
Arduino_ESP32SPI::Arduino_ESP32SPI(
    int8_t dc /* = GFX_NOT_DEFINED */, int8_t cs /* = GFX_NOT_DEFINED */,
    int8_t sck /* = GFX_NOT_DEFINED */, int8_t mosi /* = GFX_NOT_DEFINED */,
    int8_t miso /* = GFX_NOT_DEFINED */,
    uint8_t spi_num /* = VSPI for ESP32, HSPI for S2 & S3, FSPI for C3 */,
    bool is_shared_interface /* = true */)
    : _dc(dc), _spi_num(spi_num), _is_shared_interface(is_shared_interface) {
#if CONFIG_IDF_TARGET_ESP32
  if (sck == GFX_NOT_DEFINED && miso == GFX_NOT_DEFINED &&
      mosi == GFX_NOT_DEFINED && cs == GFX_NOT_DEFINED) {
...
static void _on_apb_change(void *arg, apb_change_ev_t ev_type, uint32_t old_apb,
                           uint32_t new_apb) {
  ...
  if (ev_type == APB_BEFORE_CHANGE) {
    ...
    _spi->dev->clock.val =
        spiFrequencyToClockDiv(old_apb / ((_spi->dev->clock.clkdiv_pre + 1) *
                                          (_spi->dev->clock.clkcnt_n + 1)));
    SPI_MUTEX_UNLOCK();
  }
}
...
bool Arduino_ESP32SPI::begin(int32_t speed, int8_t dataMode) {
  ...
  _buffer = (uint8_t *)heap_caps_aligned_alloc(
      16, ESP32SPI_MAX_PIXELS_AT_ONCE * 2, MALLOC_CAP_DMA);
  if (!_buffer) {
    return false;
  }

  // writeBytesDMA(...) related
  assert(_spi_num == VSPI || _spi_num == HSPI);
  spi_host_device_t host_device = _spi_num == VSPI ? VSPI_HOST : HSPI_HOST;
...
void Arduino_ESP32SPI::writeRepeat(uint16_t p, uint32_t len) {
  ...
  {
    uint16_t bufLen = (len >= ESP32SPI_MAX_PIXELS_AT_ONCE)
...
void Arduino_ESP32SPI::writePixels(uint16_t *data, uint32_t len) {
  ...
  } else // 8-bit SPI
  {
    ...
    while (len) {
      l = (len > ESP32SPI_MAX_PIXELS_AT_ONCE) ? ESP32SPI_MAX_PIXELS_AT_ONCE
                                              : len;
      l2 = (l + 1) >> 1;
...
void Arduino_ESP32SPI::writeBytes(uint8_t *data, uint32_t len) {
  ...
  } else // 8-bit SPI
  {
    ...
    while (len) {
      l = (len > (ESP32SPI_MAX_PIXELS_AT_ONCE << 1))
              ? (ESP32SPI_MAX_PIXELS_AT_ONCE << 1)
              : len;
      l4 = (l + 3) >> 2;
      p = (uint32_t *)data;
 ...
 * @param len
 */
void Arduino_ESP32SPI::writeIndexedPixels(uint8_t *data, uint16_t *idx,
                                            uint32_t len) {
  if (_dc == GFX_NOT_DEFINED) // 9-bit SPI
  {
    while (len--) {
 ...
 * @param len
 */
void Arduino_ESP32SPI::writeIndexedPixelsDouble(uint8_t *data, uint16_t *idx,
                                                  uint32_t len) {
  if (_dc == GFX_NOT_DEFINED) // 9-bit SPI
  {
    uint16_t hi, lo;
 ...
 * @return INLINE
 */
INLINE void Arduino_ESP32SPI::DC_HIGH(void) { *_dcPortSet = _dcPinMask; }
 ...
 * @return INLINE
 */
INLINE void Arduino_ESP32SPI::DC_LOW(void) { *_dcPortClr = _dcPinMask; }
